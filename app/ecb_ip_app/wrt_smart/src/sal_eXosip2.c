...
void sal_set_callbacks(Sal *ctx, const SalCallbacks *cbs){
	...
	if(ctx->callbacks.call_recv_info == NULL)//by ljw added
		ctx->callbacks.call_recv_info = (SalOnRecvINFO)unimplemented_stub;
	if(ctx->callbacks.notify_event == NULL)//2013-12-05 ljw
		ctx->callbacks.notify_event = (SalOnNotifyEvent)unimplemented_stub;
}
...
static void set_tls_options(Sal *ctx){
	if (ctx->rootCa) {
		...
		int ret = eXosip_set_tls_ctx(&tlsCtx);
		printf("[eXosip_set_tls_ctx]---ret = %d\n", ret);
		printf("eXosip_set_tls_ctx path : %s\n", tlsCtx.root_ca_cert);
	}                       
...
//	ipv6=strchr(addr,':')!=NULL;
	...
	err=eXosip_listen_addr(proto, addr, port, ipv6 ?  PF_INET6 : AF_INET, is_secure);
	ctx->running=TRUE;
	return err;
}
...
//by ljw modify
static void sdp_process(SalOp *h,int is1xx){
#if 0
	...
	if (h->sdp_offering){
		...
		if(is1xx){
			offer_answer_initiate_incoming_for_1xx(h->base.local_media,h->base.remote_media,h->result,h->base.root->one_matching_codec);
		}else
		...
		for(i=0;i<h->result->n_total_streams;++i){ //by ljw modify 2014/04/02
			if(h->result->streams[i].dir == SalStreamInactive)
				continue;
...
int sal_call_accept(SalOp * h){
	...
	if (h->base.local_media){
		...
		if (h->sdp_offering) {
			...
			/* //by ljw deleted
			if (h->sdp_answer==NULL) sdp_process(h,0); //by ljw modify
			if (h->sdp_answer){
				set_sdp(msg,h->sdp_answer);
				sdp_message_free(h->sdp_answer);
				h->sdp_answer=NULL;
...
#if 0
			//by ljw added 重新配备sdp信息。
			SalMediaDescription* newmd = NULL;
			sdp_message_t* newsdp_answer = NULL;
			if(h->sdp_answer){
				sdp_message_free(h->sdp_answer);
				h->sdp_answer = NULL;
			}
			newmd = sal_media_description_new();
			if(newmd){
				offer_answer_initiate_incoming(h->base.local_media,h->base.remote_media,newmd,h->base.root->one_matching_codec);
				newsdp_answer = media_description_to_sdp(newmd);
				if(newsdp_answer){
					set_sdp(msg,newsdp_answer);
					sdp_message_free(newsdp_answer);
					newsdp_answer = NULL;
				}
				sal_media_description_unref(newmd);
			}
			//end
#endif
		}
...
static void handle_ack(Sal *sal,  eXosip_event_t *ev){
	...
	if (op->sdp_offering){
		...
		if (sdp){
			...
			op->base.remote_media=sal_media_description_new();
			sdp_to_media_description(sdp,op->base.remote_media);
			sdp_process(op,0); //by ljw modify
			sdp_message_free(sdp);
		}
	}
	if (op->reinvite){
...
static void call_ringing(Sal *sal, eXosip_event_t *ev){
	...
	if (sdp){
		...
		sdp_to_media_description(sdp,op->base.remote_media);
		sdp_message_free(sdp);
		if (op->base.local_media) sdp_process(op,0); //by ljw modify
	}
...
int Sal_call_send_my_info(SalOp* h,char* info)
{
	osip_message_t *msg=NULL;
	char clen[10];
	char* info_body = (char*)ms_malloc(256);
	if(info_body == NULL)
		return -1;
	memset(info_body,0,256);
	strcpy(info_body,"<?xml version=\"1.0\">");
	strcat(info_body,"<wrt_contorl>");
	//strcat(info_body,"<%s></%s>",info,info);
	strcat(info_body,"<");
	strcat(info_body,info);
	strcat(info_body,"></");
	strcat(info_body,info);
	strcat(info_body,"></wrt_control>");
	eXosip_lock();
	eXosip_call_build_info(h->did,&msg);
	if (msg){
		osip_message_set_body(msg,info_body,strlen(info_body));
		osip_message_set_content_type(msg,"application/wrt_control+xml");
		snprintf(clen,sizeof(clen),"%lu",(unsigned long)strlen(info_body));
		osip_message_set_content_length(msg,clen);
		eXosip_call_send_request(h->did,msg);
		ms_warning("Sending myinfo %s request !",info);
	}
	eXosip_unlock();
	ms_free(info_body);
	return 0;
...
static void process_wrt_control_xml(Sal* sal,eXosip_event_t *ev){
	SalOp *op=find_op(sal,ev);
	osip_body_t *body=NULL;

	if (op==NULL){
		ms_warning("wrt control xml received without operation context!");
		return ;
	}
	...
	if (body && body->body!=NULL ){
		...
		ms_message("Receiving wrt_control request !");
		if (sal->callbacks.call_recv_info){
			sal->callbacks.call_recv_info(op,body->body);
	...
	if (ev->request){
		if (MSG_IS_INFO(ev->request)){
			...
			if (ct && ct->subtype){
				...
				else if(strcmp(ct->subtype,"wrt_control+xml") == 0) //by ljw added
					process_wrt_control_xml(sal,ev); //by ljw added
...
static void other_request_reply(Sal *sal,eXosip_event_t *ev){
	...
	if (ev->response){
		...
		else if(ev->request && strcmp(osip_message_get_method(ev->request),"PUBLISH") == 0){
			osip_header_t* evhdr;
			osip_message_header_get_byname(ev->request,"event",0,&evhdr);
			if(evhdr){
				SalPublishStatus status = SalPublishFailed;
				char* value = osip_header_get_value(evhdr);
				if(ev->response->status_code < 200)
					status = SalPublishProgress;
				else if(ev->response->sip_version <300 && ev->response->status_code >= 200)
					status = SalPublishOK;
				sal->callbacks.pub_reply(op,value,status);
			}
...
static int message_requires_100rel(osip_message_t *message)
{
...
int sal_register(SalOp *h, const char *proxy, const char *from, int expires){
	...
	if (h->rid==-1){
		...
		if (msg){
			...
			smartUACCore* core = sal_op_get_user_pointer(h);
			...
			osip_message_set_supported(msg, "gruu");

			/* Add our GRUU to the Contact header field */
			
			osip_message_get_contact (msg, 0, &contact_header);
			osip_contact_param_add(contact_header, osip_strdup("+sip.instance"), osip_strdup(core->gruu));
...
void sal_address_set_param(SalAddress *u,const char* name,const char* value) {
    ...
    if (param == NULL){
      ...
      //  osip_uri_uparam_add	(((osip_from_t*)u)->url,ms_strdup(name),value ? ms_strdup(value) : NULL);
	    osip_uri_uparam_add	(((osip_from_t*)u)->url,osip_strdup(name),value ? osip_strdup(value) : NULL);
    } else {
...
SalMediaDescription * sal_call_get_final_media_description_for_early_media(SalOp *h)
{
	if (h->base.local_media && h->base.remote_media /*&& !h->result*/){
		sdp_process(h,1);
	}
...
void sal_call_free_media_description(SalOp *h)
{
	if(h->result){
		sal_media_description_unref(h->result);
		h->result = NULL;
...
void sal_set_ipv4_deafult_gateway(const char* ip)
{
	eXosip_set_option(EXOSIP_OPT_SET_IPV4_FOR_GATEWAY,(void*)ip);
}
//end
