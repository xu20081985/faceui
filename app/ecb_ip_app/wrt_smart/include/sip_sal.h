...
typedef void (*SalOnPublishReply)(SalOp *salop,const char* eventName,SalPublishStatus status);
//by ljw added
typedef void (*SalOnRecvINFO)(SalOp* salop,const char* info);
typedef void (*SalOnNotifyEvent)(SalOp* salop,SalSubscribeStatus ss,const char* body,int len,const char* eventname);//2013-12-05
...
typedef struct SalCallbacks{
	...
	SalOnRecvINFO  call_recv_info; //by ljw added
	SalOnNotifyEvent notify_event; //2013-12-05
	SalOnPublishReply pub_reply;
...
int sal_manual_subscribe(SalOp *op); //by ljw modify 2013-12-11
...
/*alarm Subscribe/notify by ljw 2013-12-05*/
int sal_subscribe_alarm(SalOp *op, const char *from, const char *to);
int sal_alarm_publish(SalOp *op, const char *from, const char *to,const char* body);
int sal_unsubscribe_alarm(SalOp *op);
