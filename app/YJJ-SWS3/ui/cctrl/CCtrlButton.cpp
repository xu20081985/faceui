...
BOOL CDPButton::DoResponse(DWORD xoff, DWORD yoff, DWORD statue)
{
	...
	switch(statue)
	{
		...
		case TOUCH_UP:
			...
			if(TIMER_TIME == 1 || IR_AIR_FLAG == 1 || HEAT_FLAG == 1 || AIR_FLAG == 1 || TV_FLAG == 1 || CURTAIN_FLAG == 1 
				|| MUSIC_FALG ==1 || PWD_FLAG == 1 || SET_PWD == 1 || DIMMER_VALID_FLAG == 1) {
				if (m_afterpress != STATUS_FOCUS)
					Show(m_afterpress);
			}
...
void CDPButton::Show(DWORD statue)
{
		...
		&& (h_str != NULL))
	{
		...
		else if(m_direct == DIRECT_VERTICAL)
		{
			...
			else if(m_srcpngalign == ALIGN_MIDDLE)	// 为居中时，在分割点和底之间
...
void CDPButton::Show_Timer(DWORD statue,BOOL ishow)
{
	HANDLE h_bk;
	SIZE bksize;
	SIZE srcsize;
